...
const NoteGenerator = ({ onGenerate, setTopic }) => {
    return (
        <motion.div
            ...
            className="flex justify-center flex-col items-center mt-8 min-h-screen px-4"
        >
            <textarea
            ...
            <button
                onClick={onGenerate}
                className="mt-6 bg-beige text-dark px-4 py-2 rounded-md text-2xl sm:text-3xl font-medium transition-transform duration-300 hover:scale-105"
            >
                Generate Notes
            </button>
        </motion.div>
